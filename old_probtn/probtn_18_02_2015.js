(function ($) {
    ...
    $.fn.StartButton = function (options) {
        ...
        var ProBtnControl = {
            ...
            //curent app version
            mainVersion: "1.8.0122_09022015_dev",
            //main server url
        ...
        ProBtnControl.userDataFunction(); // = result;
        ...
        function allButton() {
            if ((ProBtnControl.userData.browserMajorVersion > "8") || (ProBtnControl.userData.browser !== "Microsoft Internet Explorer")) {
                ...
                function closeHpmdTrack() {
                    ...
                    if (params.isHPMD) {
                        try {
                            ...
                            //track opened time
                            hpmd.trackEvent("hpmd.time-expanded", { weight: ProBtnControl.contentTime.contentOpenedTime });
                        } catch (ex) {
                        }
                    }
                ...
                function MaximizeWrapper(callback) {
                    ...
                    try {
                        if (($("#probtn_button").offset().left - $(window).scrollLeft()) > pizzabtn_wrapper.width()) {
                            var newleft = pizzabtn_wrapper.width() - $("#probtn_button").width();
                            $("#probtn_button").css({ left: newleft + 'px' });
                        };
                    } catch (ex) { };
                    try {
                        if (($("#probtn_button").offset().top - $(window).scrollTop()) > pizzabtn_wrapper.height()) {
                            var newtop = pizzabtn_wrapper.height() - $("#probtn_button").height();
                            $("#probtn_button").css({ top: newtop + 'px' });
                        };
                    } catch (ex) { };
                ...
                function getFancyboxMargins() {
                    if ((params.ContentInsets.T < 0) || (params.ContentInsets.B < 0) || (params.ContentInsets.L < 0) || (params.ContentInsets.R < 0)) {
                        var isMobileLandscape = (isLandscape() && ProBtnControl.userData.mobile);
                        ...
                        if ((ProBtnControl.userData.mobile) && (params.ButtonSize.H > 70)) {
                ...
                function onOrientationChange(e) {
                    if ($("#probtn_button").position().left > (window.innerWidth - params.ButtonSize.W)) {
                        $("#probtn_button").css("left", window.innerWidth - params.ButtonSize.W);
                    }
                    if ($("#probtn_button").position().top > (window.innerHeight - params.ButtonSize.H)) {
                        $("#probtn_button").css("top", window.innerHeight - params.ButtonSize.H);
                    }
                ...
                function getSettingsAndLaunchButton(operator) {
                    if (operator === null) {
                        settingsUrl = ProBtnControl.serverUrl + "/1/functions/getClientSettings?BundleID=" + currentDomain + "&DeviceType=web&DeviceUID=" + ProBtnControl.GetDeviceUID() + "&Location[Longitude]=0&Location[Latitude]=0&Version=" + ProBtnControl.mainVersion + "&X-ProBtn-Token=" + ProBtnControl.XProBtnToken + "&random=" + Math.random() + "&ScreenResolutionX=" + ProBtnControl.userData.screenHeight + "&ScreenResolutionY=" + ProBtnControl.userData.screenWidth + "&callback=?";
                    } else {
                        settingsUrl = ProBtnControl.serverUrl + "/1/functions/getClientSettings?BundleID=" + currentDomain + "&DeviceType=web&DeviceUID=" + ProBtnControl.GetDeviceUID() + "&Location[Longitude]=0&Location[Latitude]=0&Version=" + ProBtnControl.mainVersion + "&X-ProBtn-Token=" + ProBtnControl.XProBtnToken + "&random=" + Math.random() + "&MobileOperator=" + operator + "&ScreenResolutionX=" + ProBtnControl.userData.screenHeight + "&ScreenResolutionY=" + ProBtnControl.userData.screenWidth + "&callback=?";
                    }
                    $.getJSON(settingsUrl,
                        function (data) {
                            try {
                                            ...
                                            if (inObject.hasOwnProperty(property)) {
                                                if (typeof (inObject[property]) == "string") {
                                                    try {
                                                        ...
                                                        //console.log(before + " - " + inObject[property]);
                ...
                if (window.location.protocol != "https:") {
                    cookieProbtnOrg = ProBtnControl.readCookie("probtnOrg");
                    if (cookieProbtnOrg == null) {
                        $.get("http://ip-api.com/json?fields=isp,org,as", function (response) {
                            ...
                            operator = "MTS RUS";
                            //
                            ProBtnControl.createCookie("probtnOrg", operator, (1 / 24 / 10));

                            getSettingsAndLaunchButton(operator);
                        }, "jsonp");

                    } else {
                        getSettingsAndLaunchButton(cookieProbtnOrg);
                    };
                } else {
